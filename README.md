# 欢迎访问[「伏枥之间」](https://leehenry.top)!

---

> [!TIP]
>
> ……后续更新请移步 Commit 提交记录与[「漫想与杂谈」](https://leehenry.top/archive/?tag=%F0%9F%97%83%EF%B8%8F%E5%BD%93%E6%9C%88%E4%BA%8B)系列「断点」小节的「网站建设小报」栏目。
...
- **文章卡片视觉调整优化**
  - 文章标题的高亮形式调整为下划线
- **评论区组件视觉调整优化**
  - 将 Twikoo 评论组件的样式与正文引用块相统一（包括左右外边距的一致性与响应式适配）

### ⚡ 内容个性化修改

- **修改 Profile 名片卡片的展示形式，增加手写签名**
  - 可在配置项调整是否启用

- **统一中西文均统一为 `vivo sans`，仅对西文斜体样式采用 `Open Sans`**
...
- **导航栏视觉调整优化**
    - 主页按钮与左上角 Logo 跳转逻辑重复冗余，增加主页按钮关闭的配置项；
    - 默认不显示导航栏文字，使用 hover 动画显示；
    - 默认不显示搜索框，点击右边图标弹出，与 tablet 与移动端逻辑统一；
...
- **终于调通了 Twikoo 评论组件**
    - 实现了评论主题自适应；
    - 解决了点击评论按钮导致页面自动滚动到顶部的问题。

- **首页增加「心留」留言板页面**

- **友链页面增加站点「星标」功能**，
    - `friends.js` 增加了 `star` 属性，被 star 的站点会前置展示
- **实现了主副标题通过「：」实现自动解析，并实现了样式对比区分**
    - 首页鼠标移动到主标题，会以 hover 的形式显示副标题；
    - 归档页降低副标题部分的不透明度
    - 正文页副标题以换行和改变样式的方式显示

- **增加正文页关于「字数」「阅读时长」可见性配置项**
    - 默认关闭，可以在 `config.ts` 中控制
...
- **取消所有圆角**！不甘平庸，做圆滑当道的锐利异类。
    - 修改包括但不限于页面组建卡片、图片、代码块、引用块、热力图以及一切按钮，用锐利的方形作为内容的分隔；

- **取消所有非必要的颜色**！设计回归内容本身，淡化附属功能组件，去除一切非必要的强调。
    - 取消内容标签、附属按钮的高亮强调；
    - 取消超链接的高亮强调，加粗下划线；
    - 取消许多按钮默认的颜色高亮；
    - 增加内容粗体的高亮强调，**让该有的高亮更高亮**。
